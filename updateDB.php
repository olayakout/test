<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('employees', "create table if not exists `employees` (   `EmployeeID` INT not null auto_increment , primary key (`EmployeeID`), `TitleOfCourtesy` VARCHAR(50) , `Photo` VARCHAR(40) , `LastName` VARCHAR(50) , `FirstName` VARCHAR(10) , `Title` VARCHAR(30) , `BirthDate` DATE , `age` INT , `HireDate` DATE , `Address` VARCHAR(50) , `City` VARCHAR(15) , `Region` VARCHAR(15) , `PostalCode` VARCHAR(10) , `Country` VARCHAR(15) , `HomePhone` VARCHAR(24) , `Extension` VARCHAR(4) , `Notes` TEXT , `ReportsTo` INT ) CHARSET latin1", $silent);
		...
		setupTable('logs', "create table if not exists `logs` (   `id` INT unsigned not null auto_increment , primary key (`id`), `ip` VARCHAR(16) , `ts` BIGINT , `details` TEXT ) CHARSET latin1", $silent);
		setupTable('test', "create table if not exists `test` (   `id` INT unsigned not null auto_increment , primary key (`id`), `name` VARCHAR(40) , `description` TEXT ) CHARSET latin1", $silent, array( "ALTER TABLE `table10` RENAME `test`","UPDATE `membership_userrecords` SET `tableName`='test' where `tableName`='table10'","UPDATE `membership_userpermissions` SET `tableName`='test' where `tableName`='table10'","UPDATE `membership_grouppermissions` SET `tableName`='test' where `tableName`='table10'","ALTER TABLE test ADD `field1` VARCHAR(40)","ALTER TABLE `test` CHANGE `field1` `id` VARCHAR(40) ","ALTER TABLE `test` CHANGE `id` `id` INT unsigned not null auto_increment ","ALTER TABLE test ADD `field2` VARCHAR(40)","ALTER TABLE `test` CHANGE `field2` `name` VARCHAR(40) ","ALTER TABLE test ADD `field3` VARCHAR(40)","ALTER TABLE `test` CHANGE `field3` `description` VARCHAR(40) ","ALTER TABLE `test` CHANGE `comments` `comments` TEXT "));
